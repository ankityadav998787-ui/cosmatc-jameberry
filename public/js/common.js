...
// Backend base URL. Empty = same origin (single service). See js/config.js for split hosting.
const API_BASE = (window.GLOW_API_BASE || '').replace(/\/$/, '');
...
async function api(path, opts = {}) {
  ...
  try {
    ...
    throw new Error('Cannot reach the server. Is the backend (server.js) running?');
  }
  if (res.status === 401) {
    localStorage.removeItem('glow_token');
  }
  if (res.status === 404) {
    throw new Error('API not found (404). The backend is not running at this address — see js/config.js.');
  }
  if (!res.ok) {
